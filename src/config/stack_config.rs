...
use crate::{
    common::freqs::FreqInfo, config::stack_config_soapy::CfgSoapySdr,
    entities::lmac::components::scramble::scrambler,
};
        ...
        match self.phy_io.backend {
            PhyBackend::SoapySdr => {
                ...
                let config_count = [
                    ...
                    soapy_cfg.io_cfg.iocfg_bladerf.is_some(),
                ]
                .iter()
                .filter(|&&x| x)
                .count();
                if config_count != 1 {
                    return Err(
                        "soapysdr backend requires exactly one hardware configuration (iocfg_usrpb2xx, iocfg_limesdr, iocfg_sxceiver, or iocfg_bladerf)",
                    );
                }
            }
